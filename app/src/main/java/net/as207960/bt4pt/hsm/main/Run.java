...
public class Run {
    public static void main(String[] args) {
        ...
        requireSuccess(simulator.transmitCommand(
                new CommandAPDU(0x80, 0x10, 0x01, 0x00)));

        ResponseAPDU publicKey = simulator.transmitCommand(
                new CommandAPDU(0x80, 0x11, 0x01, 0x00, 256));
        requireSuccess(publicKey);
        System.out.println("EC public key: " + toHex(publicKey.getData()));

        ResponseAPDU signature = simulator.transmitCommand(
                new CommandAPDU(0x80, 0x21, 0x00, 0x00, "bt4pt".getBytes(StandardCharsets.UTF_8)));
        requireSuccess(signature);
        System.out.println("EC_KNR signature r||s||remainder: " + toHex(signature.getData()));
